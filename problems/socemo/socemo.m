function Y_ND = socemo(problem, num_objectives, maxeval)% surrogate, n_start, init_design, sampling, own_design)
...
datafile = "datainput_mop1"; % this file calls the relevant method for the problem
Data=feval(datafile,problem, num_objectives); % load problem data
...
Data.maxeval  = double(maxeval); %total number of objective function evaluations; every f(x) will be evaluated 500 times
...
eval_counter = 0;
maxcomp = 450;
while eval_counter + Data.m <= maxcomp %sample until we run out of function evaluation budget
    ...
    eval_counter = eval_counter + size(Data.S,1);
end
disp(eval_counter);

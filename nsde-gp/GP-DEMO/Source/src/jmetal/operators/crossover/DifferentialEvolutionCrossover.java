...
public class DifferentialEvolutionCrossover extends Crossover {
   ...
   */
  private static List VALID_TYPES = Arrays.asList(RealSolutionType.class, ArrayRealSolutionType.class, DiscreteSolutionType.class) ;
    ...
    if (!(VALID_TYPES.contains(parent[0].getType().getClass()) &&
		...
		child = new Solution(current) ;
                ...
                /*DEMO modifikacija! Osebki morajo biti paroma različni */
		...
		if ((DE_Variant_.compareTo("rand/1/bin") == 0) || 
			...
			for (int j=0; j < numberOfVariables; j++) {
				if (PseudoRandom.randDouble(0, 1) < CR_ || j == jrand) {
					double value ;
					value = xParent2.getValue(j)  + F_ * (xParent0.getValue(j) -   xParent1.getValue(j)) ;            /*DEMO modifications */
					
					if(current.getType().getClass() == DiscreteSolutionType.class)
						value = getDiscretizedValue(value, Params.getDiscretizationStep(j));

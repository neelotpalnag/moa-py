 ...
 */
public class RankingUnderUncertantyZaStPrimerjavZaBBox {
  ...
  public int[] SteviloPravilnihPrimerjavZBB = new int[3];
  public int[] SteviloNepravilnihPrimerjavZBB = new int[6];
  public int steviloDodatnihEkzaktnihIzracunov;
  public int steviloPrimerjavZDodatnimiIzracuni;
  public double povprecnaSirinaIntervala;
  
  Model modelGlobal_;
  static Problem problem;
  SolutionSet populationZaPrimerjanjePrimerjav;
  SolutionSet pomozniPopulationZaPrimerjanjePrimerjav;
  ...
  public  RankingUnderUncertantyZaStPrimerjavZaBBox(SolutionSet population, Problem problem_, Model model, Problem prob) throws JMException, ClassNotFoundException {        
    
  	steviloPrimerjav =0;
  	inicializirajTebeloNaNic(SteviloPravilnihPrimerjavZBB);
  	inicializirajTebeloNaNic(SteviloNepravilnihPrimerjavZBB);
  	steviloDodatnihEkzaktnihIzracunov = 0;
  	steviloPrimerjavZDodatnimiIzracuni = 0;
  	povprecnaSirinaIntervala = 0;
  	modelGlobal_ = model;
  	problem = createProblem(prob.getName());
  	populationZaPrimerjanjePrimerjav = new SolutionSet(200);
  	pomozniPopulationZaPrimerjanjePrimerjav = new SolutionSet(200);
  	int neprimerljivi = 0;
  	
  	narediPrimernoKopijoPopulacijeZaNOBB(populationZaPrimerjanjePrimerjav, population);
  	
  	prepisiPrviSetVDrugega(populationZaPrimerjanjePrimerjav, pomozniPopulationZaPrimerjanjePrimerjav);
    ...
    // of individuals that dominate me
      ...
      int kolikoDodatnihIzracunovJeBiloPotrebno = 0;
      	if(p != q)
      	{
      	  steviloPrimerjav++;  
      	  //if(p==196 && q == 15)
      	  	//System.out.print("");
      	  //prepisiPrviSetVDrugega(pomozniPopulationZaPrimerjanjePrimerjav, populationZaPrimerjanjePrimerjav);
	        ...
	        //za toènost primerjav z BB
	        boolean ponavljaj = true;
	        ...
	        while(ponavljaj)
      		{
			        ...
			        {
			        	if (flagDominate == -1) {       //if P dominates Q
			        		SteviloPravilnihPrimerjavZBB[0]++;	//iDominate[p].add(new Integer(q)); //Add q to the set of solutions dominatet by p
				        } 
				        
				        else if (flagDominate == 1) {
				        	SteviloPravilnihPrimerjavZBB[1]++;	//dominateMe[p]++;  //Domination counter of p 
				        }
				        
				        else if(flagDominate == 0){
				        	SteviloPravilnihPrimerjavZBB[2]++;
				        }
			        	ponavljaj =false;
			        }
			        ...
			        {
			        	if(flagDominateNOBB == 1 || flagDominateNOBB == -1 | flagDominateNOBB == 0)	//napaka
			        	{
					        	if (flagDominate == -1) {       //if P dominates Q
					        		if(flagDominateNOBB == 1)
					        			SteviloNepravilnihPrimerjavZBB[0]++;		//napoved pove q<p v resnici pravilno je p<q
					        		else
					        			SteviloNepravilnihPrimerjavZBB[1]++;
						        } 
						        
						        else if (flagDominate == 1) {
						        	if(flagDominateNOBB == -1)
						        		SteviloNepravilnihPrimerjavZBB[2]++;		//napoved pove p<q v resnici pravilno je q<p
					        		else
					        			SteviloNepravilnihPrimerjavZBB[3]++;
						        }
						        
						        else if(flagDominate == 0){
						        	if(flagDominateNOBB == -1)
						        	{
						        		SteviloNepravilnihPrimerjavZBB[4]++;		//napoved pove p<q v resnici pravilno je p||q
						        		//if(populationZaPrimerjanjePrimerjav.get(p).isExactllyEvaluated() == false && populationZaPrimerjanjePrimerjav.get(q).isExactllyEvaluated() == false)
						        			//System.out.println("BB: p = "+p+"  q="+q);
						        	}
					        		else
					        			SteviloNepravilnihPrimerjavZBB[5]++;
						        }
			        	...
			        	else	//negotovost
			        	{
			        		...
			        		if (flagDominateNOBB == 2 || flagDominateNOBB == 10){
			             	
			            	problem_.evaluate(populationZaPrimerjanjePrimerjav.get(q)) ;
			            	populationZaPrimerjanjePrimerjav.get(q).setStandardDevianceToZero();
			              problem_.evaluateConstraints(populationZaPrimerjanjePrimerjav.get(q));      
			              populationZaPrimerjanjePrimerjav.get(q).setExactllyEvaluated(true);	
			              steviloDodatnihEkzaktnihIzracunov++;
			              kolikoDodatnihIzracunovJeBiloPotrebno ++;
			            }
			        		
			        		else {
			        			problem_.evaluate(populationZaPrimerjanjePrimerjav.get(p)) ;
			            	populationZaPrimerjanjePrimerjav.get(p).setStandardDevianceToZero();
			              problem_.evaluateConstraints(populationZaPrimerjanjePrimerjav.get(p));      
			              populationZaPrimerjanjePrimerjav.get(p).setExactllyEvaluated(true);
			              steviloDodatnihEkzaktnihIzracunov++;
			              kolikoDodatnihIzracunovJeBiloPotrebno ++;
			        		}
      	...
      	if(kolikoDodatnihIzracunovJeBiloPotrebno > 0)
      	{
      		steviloPrimerjavZDodatnimiIzracuni++;

      	  //prepisemo, da ima set vedno vse elemente aproksimirane
      	  prepisiPrviSetVDrugega(pomozniPopulationZaPrimerjanjePrimerjav, populationZaPrimerjanjePrimerjav, p, q);
      	}      	
    ...
    //System.out.println("Neprimerljiva:  "+ neprimerljivi);
  } // Ranking

  
  private void prepisiPrviSetVDrugega(SolutionSet originalSet,  SolutionSet kopijaSet) {
	   

  	kopijaSet.clear();
  	for(int i=0; i<originalSet.size();i++)
  		kopijaSet.add(new Solution(originalSet.get(i)));
	  
  }

  private void prepisiPrviSetVDrugega(SolutionSet originalSet,  SolutionSet kopijaSet, int p, int q) {
	   
  	kopijaSet.replace(p, new Solution(originalSet.get(p)));
  	kopijaSet.replace(q, new Solution(originalSet.get(q)));
  		//kopijaSet.add(new Solution(originalSet.get(i)));
	...
	private void narediPrimernoKopijoPopulacijeZaNOBB(SolutionSet populationZaPrimerjanje, SolutionSet population) throws ClassNotFoundException, JMException {
  	...
  	for(int i=0; i< population.size();i++)
  	{
    	...
    	s.setExactllyEvaluated(false);
  		populationZaPrimerjanje.add(s); 
  		
  		sirina= s.getstandardDeviance(0);
  		if(Double.isNaN(sirina))
  			stIzpuscenih++;
  		else
  			povprecnaSirinaIntervala = povprecnaSirinaIntervala + sirina;
  		
  		sirina= s.getstandardDeviance(1);
  		if(Double.isNaN(sirina))
  			stIzpuscenih++;
  		else
  			povprecnaSirinaIntervala = povprecnaSirinaIntervala + sirina;
  	...
  	povprecnaSirinaIntervala =povprecnaSirinaIntervala/(population.size() - stIzpuscenih);
  }

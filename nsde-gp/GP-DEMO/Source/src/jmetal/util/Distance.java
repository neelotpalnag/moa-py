  ...
  */
  public void crowdingDistanceAssignment(SolutionSet solutionSet, int nObjs) {
    ...
    for (int i = 0; i<nObjs; i++) {          
  	...
  	//Tejin dodatek ki spremeni crowding distance kjer imamo enake rešitve
		//preverimo èe imamo kje podvojene rešitve		
		try {
		...
		for (int i = 0; i<size-1; i++) 
		{
			...
			{
				if(distanceBetweenSolutions(front.get(i), front.get(j)) == 0)
        {
					//vsota obeh distanc se zapiše k drugi rešitvi
					distance = front.get(i).getCrowdingDistance() + front.get(j).getCrowdingDistance();                
	        front.get(j).setCrowdingDistance(distance);
	        front.get(i).setCrowdingDistance(0);
        	break;
        }   
			}
		}

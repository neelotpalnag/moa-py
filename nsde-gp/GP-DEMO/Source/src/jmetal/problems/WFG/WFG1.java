...
public class WFG1 extends WFG {
  ...
  */
  ...
  public void evaluateConstraints(Solution solution) throws JMException {
  	...
  	int  solutionFeasibility; 	//-1 = (probably) infeasible. 0 = uncertainly feasible.   1 = (probably) feasible 
  	
  	  	
  	distanceToFisibility =  calculateDistanceToFeasibility(solution); 
  	solutionFeasibility = calculateSolutionFeasibility(solution, distanceToFisibility);
  	solution.setSolutionFeasibility(solutionFeasibility);
  	
  	if(distanceToFisibility != 0)
  	{
  		if(solutionFeasibility == 0)	//in order to be able to distinguish and compare between infeasible and possible feasible solutions
  		{
  ...
  private int calculateSolutionFeasibility(Solution solution,  double distanceToFisibility) {

  	
  	if(distanceToFisibility == 0)	//feasible
  	{
	  	for(int i=0; i< solution.numberOfObjectives(); i++)
			{
				if(solution.getObjective(i) - solution.getstandardDeviance(i) < 0)	//0 = lower boarder
					return 0;
			}
	  	return 1;
	  }
	...
	private double calculateDistanceToFeasibility(Solution solution) {
		...
		double distance = 0;
		int  solutionFeasibility = 1;
		
		for(int i=0; i< solution.numberOfObjectives(); i++)
		{
			if(solution.getObjective(i) < 0)
			{
				distance = distance - solution.getObjective(i);
				
			}
			if(solution.getObjective(i) - solution.getstandardDeviance(i) < 0)
  ...
  */  
  public final void evaluate(Solution solution) throws JMException {
    ...
    //solution.setStandardDevianceToZero();

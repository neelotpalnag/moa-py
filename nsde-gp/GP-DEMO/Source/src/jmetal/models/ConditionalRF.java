 ...
 */
public class ConditionalRF extends Model {  
   ...
   * @throws Exception 
   */
  public ConditionalRF(int noInput, int noOutput) throws Exception {
	  ...
	  String[] Rags = {"--vanilla"};
	  rengine = new Rengine(Rags, false, new CallbackListener());
	  rengine.eval("library(party)");
  ...
  public void evaluate(Solution solution) throws JMException, ClassNotFoundException {
	  ...
	  //za vse testne podatke zgradimo data frame v R-ju
		...
		*/
		...
		for(int i=0; i< numberOfObjectives_;i++)
		{
			 lsMakePrediction ="predictions" + i + " <- predict(model" + i + ", newdata=dfm" + i + ", type='prob')";	//dfPredict
		...
		for(int i=0; i< numberOfObjectives_;i++)
		{
			 ...
			 result =rengine.eval("predictions" + i);		//result = rengine.eval(lsgetMean);
			 ldObjectiveValuePredictions[i] = result.asList().at(0).asDouble();		//result.asDoubleArray()[0];   // na niè, ker imamo le eno predikcijo rešitve
		...
		for(int i=0; i< numberOfObjectives_;i++)
		{
			...
			ldObjectiveValueStdDev[i] = 0;		//result.asDoubleArray()[0];
  ...
  public void update(Solution solution) {
	...
	private void buildClassifiers() {
	  ...
	  //za vse testne podatke zgradimo data frame v R-ju
		...
		for(int i=0; i< numberOfObjectives_; i++)
		{
			lsCreateModel = "model" + i + " <- cforest(objective" + i + " ~.,data=df" + i + ", control = cforest_unbiased(ntree=2000))";

 ...
 */
public class SPGPHT extends Model {
   ...
   */
  public SPGPHT(int noInput, int noOutput, int noPseudo) throws Exception {
  	...
  	hs_ = new MWNumericArray[noOutput];
  	...
  	for (int i = 0; i < hypLength_; i++)
		...
		for (int i = 0; i < noOutput; i++) {
  		...
  		hs_[i] = new MWNumericArray(MWClassID.DOUBLE);
		}
		fixed_ = false;
  	// set data
  ...
  public void evaluate(Solution solution) throws JMException, ClassNotFoundException {
		...
		for (int i = 0; i < numberOfOutputs_; i++) {
			try {
				...
				output = spgp_.pred_ht(3, toArray(outputsAll_[i]), toArray(inputsAll_), bvs_[i], toArray(solution.getDecisionVariables()), hyp_[i], hs_[i], 0, 0);
		      ...
		      solution.setStandardDeviance(i, Math.sqrt(variance) * 1);
				}
				else {
		      if (solution.numberOfFeatures() > 0)
		...
		for (int i = 0; i < numberOfOutputs_; i++) {
			...
			while (true) {
				try {
					c++;
					if (c > 1) {
						output = spgp_.initHyp_ht(1, toArray(outputs_[i]), toArray(inputs_[i]), Math.min(numberOfPseudoInputs_, inputs_[i].size()), Math.min(c * 0.2, 3));
					}
					else {
						if (fixed_ || !resetHyp_)
							output = spgp_.initHyp_ht(1, toArray(outputs_[i]), toArray(inputs_[i]), Math.min(numberOfPseudoInputs_, inputs_[i].size()), hyp_[i]);
						else
							output = spgp_.initHyp_ht(1, toArray(outputs_[i]), toArray(inputs_[i]), Math.min(numberOfPseudoInputs_, inputs_[i].size()));
					}
					theta = ((MWNumericArray)output[0]).getDoubleData();
					theta = minimize(i, theta);
					output = spgp_.unwrap_ht(3, transpose(theta), numberOfInputs_);
					bvs_[i] = (MWNumericArray)output[0];
					if (!fixed_)
						hyp_[i] = (MWNumericArray)output[1];
					hs_[i] = (MWNumericArray)output[2];
					break;
				}
				catch (Exception e) {
					//e.printStackTrace();
		  ...
		  //output = spgp_.pred(4, toArray(outputs_[i]), toArray(inputs_[i]), bvs_[i], bvs_[i], hyp_[i], 0, 0);
			output = spgp_.bv_ht(2, toArray(outputs_[i]), toArray(inputs_[i]), bvs_[i], hyp_[i], hs_[i], 0);
	...
	private double[] minimize(int index, double[] theta) throws Exception {
		...
		w = (double[])output[0];
		f = (double[])output[1];
		System.out.printf("SPGPHT-%2d: %4.6f --> %4.6f\n", index, f[0], f[f.length-1]);
		return w;
	}
	...
	private Object[] minimize(double[] X, int index) throws Exception {
		...
		ls_failed = false;
		if (fixed_)
			output = spgp_.lik_ht_nohyp(7, transpose(X), toArray(outputs_[index]), toArray(inputs_[index]), 0, 0);
		else
			output = spgp_.lik_ht(7, transpose(X), toArray(outputs_[index]), toArray(inputs_[index]), 0, 0);
		if (!((MWCharArray)output[6]).toString().equalsIgnoreCase("OK"))
			throw new Exception(((MWCharArray)output[6]).toString());
			...
			while (true) {
				...
				while (!success && M > 0) {
					try {
						...
						if (fixed_)
							output = spgp_.lik_ht_nohyp(7, transpose(sumVectors(X, multiplyVector(s, x3))), toArray(outputs_[index]), toArray(inputs_[index]), 0, 0);
						else
							output = spgp_.lik_ht(7, transpose(sumVectors(X, multiplyVector(s, x3))), toArray(outputs_[index]), toArray(inputs_[index]), 0, 0);
						if (!((MWCharArray)output[6]).toString().equals("OK"))
							throw new Exception();
						if (isAnyNanOrInf(((MWNumericArray)output[0]).getDoubleData()) || isAnyNanOrInf(((MWNumericArray)output[1]).getDoubleData()) || isAnyNanOrInf(((MWNumericArray)output[2]).getDoubleData()) || isAnyNanOrInf(((MWNumericArray)output[3]).getDoubleData()) || isAnyNanOrInf(((MWNumericArray)output[4]).getDoubleData()) || isAnyNanOrInf(((MWNumericArray)output[5]).getDoubleData()))
							throw new Exception();
		...
		System.out.printf("SPGPHT optimization ended after %d/%d iterations\r\n", Math.abs(i), maxOptIter_);

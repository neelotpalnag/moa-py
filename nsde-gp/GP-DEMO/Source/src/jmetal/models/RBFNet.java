 ...
 */
public class RBFNet extends Model {  
  
	RBFNetwork[] tableOfRFClassifiers;
   ...
   * @throws Exception 
   */
  public RBFNet(int noInput, int noOutput) throws Exception {
	  ...
	  tableOfRFClassifiers = new RBFNetwork[numberOfObjectives_];
  ...
  public void update(SolutionSet solutions) {
    ...
    {
			try {
		    addSolutionToTrainingSet(solutions.get(i));
	    } 
			catch (JMException e) {
		    e.printStackTrace();
	    }	  
  ...
  public void evaluate(Solution solution) throws JMException, ClassNotFoundException {
		...
		Instance iExample;
		double[] distributionForInstance; // = new double[];
		...
		for(int i=0; i< numberOfObjectives_; i++)
		{
			...
			iExample.setDataset(tableOfSolutionsForEachObjective[i]);

			
			//approximate
			try {
				approximation = tableOfRFClassifiers[i].classifyInstance(iExample);
	...
	private void initializeRBF()
	{		
		for(int i=0; i<numberOfObjectives_; i++)
		{
			tableOfRFClassifiers[i] = new RBFNetwork();
			//tableOfRFClassifiers[i].
			tableOfRFClassifiers[i].setClusteringSeed((int)Params.getSeed());
		}
	}
} // RF
